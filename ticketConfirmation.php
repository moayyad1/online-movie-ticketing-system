...
//First input in database table
	if (!$dateExists) {
		$sql="INSERT INTO showOrder(showOrderId,date,timeslot,theater,movieName,seat,user)  VALUES ('','".$date."','". $time. "','".$theater."','".$movieName."',".'50'.",'".$user."');";



		if ($conn->query($sql) === TRUE) {
			$result=$conn->query("select showOrderId from showOrder where date='".$date."' and timeslot='".$time."' and theater='".$theater."' and movieName='".$movieName ."' and user='".$user."' ;");
...
// database e date time theater sob e ase , ekhon movie ase kina check korbo.
			...
			$resourse=$conn->query("select movieName from showOrder where date='".$date."' and timeslot='".$time."' and theater='".$theater."' and user='".$user ."';");
			$movieExist=false;
			while ($rw=$resourse->fetch_object()) {
...
//sob kichur input dewa ase,ekhon seat shudhu kombe
			$result=$conn->query("select showOrderId from showOrder where date='".$date."' and timeslot='".$time."' and theater='".$theater."' and user='".$user."';");
							...
							<table class="table table-user-information">
								<tbody>
									...
									<form action="seatReducing.php" method="post">
										...
										<input type="hidden" name="theater" value=<?php echo '"'.$user.'"'; ?>>
